...
export async function POST(request: NextRequest) {
  try {
    ...
    const startRes = await fetch(`${process.env.NODE_ENV === 'production' ? '/start-discovery' : 'http://127.0.0.1:8000/start-discovery'}`, {
      method: "POST",
    ...
    const statusRes = await fetch(`${process.env.NODE_ENV === 'production' ? '/discovery-status' : 'http://127.0.0.1:8000/discovery-status'}/${start.job_id}`)
